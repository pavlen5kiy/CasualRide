...
def close(*args):
    ...
    song, coins, main_skin, road_skin, skins = args
...
def play():
    ...
    car = Car(load_image(f'car_{list(skins.keys())[main_skin]}_up'), road, npc_cars, player)
        ...
        for event in pygame.event.get():
            ...
            if skins[list(skins.keys())[car_skin]][0] == 'locked':
                if lock_button.update(event):
                    if coins_count.coins_count - skins[list(skins.keys())[car_skin]][1] >= 0:
                        skins[list(skins.keys())[car_skin]][0] = 'open'
                        main_skin = car_skin
                        coins_count.coins_count = coins_count.coins_count - skins[list(skins.keys())[car_skin]][1]
...
if __name__ == '__main__':
    ...
    main_menu()
    close(current_song, coins_count.coins_count, main_skin, road_skin, skins)
